...
class ScanDefPanel(wx.Panel):
    ...
    def fill_rows(self):
        ...
        for scan in self.scandb.getall('scandefs',
            ...
            self.wids.AppendItem((scan.name, axis, npts, mtime, utime))
        ...
        if self.wids.HasSelection():
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if scans is not None:
                    for s in scans:
                        if s.name == name and s.type == self.stype:
                            self.scandb.rename_scandef(s.id, newname)
                    self.scandb.commit()
                    #wx.CallAfter(self.fill_rows)

                store = self.wids.GetStore()
                # print 'RENAMED  to ', newname
                # print dir(store)
                #store.SetValueByRow(row, 0, newname)
        ...
        if self.wids.HasSelection():
            ...
            if ret == wx.ID_YES:
                ...
                if scans is not None:
                    for s in scans:
                        if s.name == name and s.type == self.stype:
                            self.scandb.del_scandef(scanid=s.id)
                    self.scandb.commit()
                    #wx.CallAfter(self.fill_rows)
...
class MeshScanDefs(ScanDefPanel):
    ...
    def fill_rows(self):
        ...
        for scan in self.scandb.getall('scandefs',
            ...
            mtime = scan.modify_time.strftime("%Y-%b-%d %H:%M")
            utime = scan.last_used_time.strftime("%Y-%b-%d %H:%M")
            self.wids.AppendItem((scan.name, inner, outer, npts, mtime, utime))
...
class XAFSScanDefs(ScanDefPanel):
    ...
    def fill_rows(self):
        ...
        for scan in self.scandb.getall('scandefs',
            ...
            mtime = scan.modify_time.strftime("%Y-%b-%d %H:%M")
            utime = scan.last_used_time.strftime("%Y-%b-%d %H:%M")
            self.wids.AppendItem((scan.name, e0, nreg, npts, mtime, utime))
